...
CREATE TABLE IF NOT EXISTS `exe_36`.`user` (
  ...
  `permission_idpermission1` SMALLINT UNSIGNED NOT NULL,
  PRIMARY KEY (`iduser`, `permission_idpermission`),
  UNIQUE INDEX `login_UNIQUE` (`login` ASC),
  INDEX `fk_user_permission1_idx` (`permission_idpermission1` ASC),
  CONSTRAINT `fk_user_permission1`
    FOREIGN KEY (`permission_idpermission1`)
...
CREATE TABLE IF NOT EXISTS `exe_36`.`news` (
  ...
  `user_permission_idpermission` SMALLINT UNSIGNED NOT NULL,
  PRIMARY KEY (`idnews`),
  INDEX `fk_news_user1_idx` (`user_iduser` ASC, `user_permission_idpermission` ASC),
  CONSTRAINT `fk_news_user1`
    FOREIGN KEY (`user_iduser` , `user_permission_idpermission`)
    REFERENCES `exe_36`.`user` (`iduser` , `permission_idpermission`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
